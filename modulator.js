...
function nysiis(s, truncate) {
  truncate = truncate != null ? truncate : true;
  ...
  return truncate && s.length > 6 ? s.substring(0, 6) : s;
}
...
nysiis.run = function (s, rules) {
  for (var i = 0; i < rules.length; i++) {
    s = s.replace(rules[i].pattern, rules[i].value);
  }
    ...
    (() => {
      ...
      for (let i = 0; i < matches.length; i++) {
        if (matches[i].trim() !== "") {
          ...
          inter = nysiis(inter, false).toLowerCase().replaceAll("undefi", "");
          inter = inter
            .replaceAll("ea", "ee")
            .replaceAll("is", "iz")
            .replaceAll("ike", "iek")
