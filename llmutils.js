...
async function runPrompt(prompt) {
  const res = await runCommand(
    ...
    // `llama.cpp/build/bin/main -m models/13bpq/pyg.bin -e -p "${prompt}" -n 48 -b 1024 -c 2048 --top_p 0.8 --temp 0.8 --repeat-penalty 1.08 --mirostat 2 --prompt-cache-all --prompt-cache mirostatcache --no-penalize-nl`
  );
  return res;
}
